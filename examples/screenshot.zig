...
pub fn main() !void {
    ...
    try setup();
    errdefer _ = Screen.disableRawMode() catch {
        std.log.err("error disabling raw mode", .{});
    };
    defer cleanup() catch {
        std.log.err("error cleaning up terminal", .{});
    };
    ...
    while (true) {
        if (try stream.parseEvent()) |event| {
            switch (event) {
                .key => |key| {
                    if (key.matches(&.{ .{ .code = .char('q') }, .{ .code = .char('c'), .ctrl = true }, .{ .code = .char('C'), .ctrl = true } })) break;
                },
                .resize => |resize| {
...
const App = struct {
    ...
    pub fn render(buffer: *zuit.Buffer, area: zuit.Rect) !void {
        const hoz = Layout(3).horizontal(&.{ .{ .fill = 1 }, .{ .length = 60 }, .{ .fill = 1 } })
            .split(area)[1];
        const vert = Layout(3).vertical(&.{ .{ .fill = 1 }, .{ .length = 20 }, .{ .fill = 1 } })
            .split(hoz)[1];

        const b = widget.Block {
            .borders = .all,
            .padding = .proportional(1),
        };
        try b.render(buffer, vert);

        const hl = Style{ .bg = .yellow, .fg = .black };
        const p = widget.Paragraph{
            .lines = &.{
                .init(&.{ .raw(c[0]), .styled(c[1], hl), .raw(c[2]), .styled(c[3], hl), .raw(c[4]) }),
            },
            .text_align = .center,
            .trim = true,
            .wrap = true,
        };
        try p.render(buffer, b.inner(vert));
    }
};
